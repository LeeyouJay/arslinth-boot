<template>
	<div class="auth-select">
		<div class="auth-checke" :style="{'text-align':positiion}">
			...
			<el-checkbox :value="strictly" @change="handleCheckedTreeConnect">父子联动</el-checkbox>
		</div>
		<div class="auth-tree">
			<el-tree :data="tableTree" node-key="name" ref="tree" empty-text="加载中，请稍候..." show-checkbox
				:check-strictly="!strictly" default-expand-all @check="checkChange">
				<span class="custom-tree-node" slot-scope="{ node, data }">
					<span style="line-height: 2;">{{node.label}}</span>
					<span><el-button size="mini" type="text" :class="data.isFunc ?'red':'blue'">{{data.name}}</el-button> </span>
				</span>
				</el-tree>
		</div>
	</div>
