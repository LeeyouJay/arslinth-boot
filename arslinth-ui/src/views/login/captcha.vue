	...
	export default {
		...
		methods: {
			async init() {
				...
				this.initDom()
				this.bindEvents()
				this.widthlable = 'width:' + this.catcha.imgW + 'px;'
			},
			initDom() {
			...
			initImg() {
				...
				img.onload = onload
				img.onerror = () => {
					img.src = that.catcha.imgurl
				}
				img.src = that.catcha.imgurl
				img.onload = function() {
					that.canvasCtx.drawImage(img, 0, 0, that.catcha.imgW, that.catcha.imgH)
				}
			...
			bindEvents() {
				document.addEventListener('mousemove', e => {
					...
					if (moveX < 0 || moveX + 38 >= this.catcha.imgW) return false
					this.sliderLeft = moveX + 'px'
					const blockLeft = ((this.catcha.imgW - 40 - 20) / (this.catcha.imgW - 40)) * moveX
